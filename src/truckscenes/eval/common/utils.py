...
from truckscenes.eval.common.data_classes import EvalBox
...
def attr_acc(gt_box: DetectionBox, pred_box: DetectionBox) -> float:
    ...
    If the GT class has no attributes or the annotation is missing attributes,
    we assign an accuracy of nan, which is
    ignored later on.

    Arguments:
        gt_box: GT annotation sample.
        pred_box: Predicted sample.

    Returns:
        acc: Attribute classification accuracy (0 or 1) or
            nan if GT annotation does not have any attributes.
    """
    if gt_box.attribute_name == '':
        # If the class does not have attributes or this particular
        # sample is missing attributes, return nan, which is
        # ignored later. Note that about 0.4% of the sample_annotations
        # have no attributes, although they should.
        acc = np.nan
    else:
...
def scale_iou(sample_annotation: EvalBox, sample_result: EvalBox) -> float:
    ...
    if we assume that the boxes are aligned, i.e. translation and
    rotation are considered identical.

    Arguments:
        sample_annotation: GT annotation sample.
        sample_result: Predicted sample.
...
def quaternion_yaw(q: Quaternion) -> float:
    ...
    Note that this only works for a quaternion that represents a box in
    lidar or global coordinate frame.

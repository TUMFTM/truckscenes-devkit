...
class EvalBox(abc.ABC):
    ...
    @classmethod
    @abc.abstractmethod
    def deserialize(cls, content: Dict[str, list]):
        pass
...
class EvalBoxes:
    ...
    def __init__(self):
        ...
        """
        self.boxes: Dict[str, List[EvalBox]] = defaultdict(list)
    ...
    def add_boxes(self, sample_token: str, boxes: Dict[str, List[EvalBox]]) -> None:
        """ Adds a list of boxes. """
        self.boxes[sample_token].extend(boxes)

    def serialize(self) -> Dict[str, List[EvalBox]]:
        """ Serialize instance into json-friendly format. """
        return {key: [box.serialize() for box in boxes] for key, boxes in self.boxes.items()}

    @classmethod
    def deserialize(cls, content: Dict[str, List[EvalBox]], box_cls: EvalBox):
        """
...
class MetricData(abc.ABC):
    ...
    @classmethod
    @abc.abstractmethod
    def deserialize(cls, content: Dict[str, List[EvalBox]]):
        """ Initialize from serialized content. """
        pass

...
from truckscenes.eval.common.data_classes import MetricData, EvalBox
from truckscenes.eval.common.utils import center_distance
from truckscenes.eval.detection.constants import DETECTION_NAMES, ATTRIBUTE_NAMES, TP_METRICS
...
class DetectionMetricData(MetricData):
    """ This class holds accumulated and interpolated data required to
    ...
    def __init__(self,
        ...
        # Assert ordering.
        # Confidences should be descending, Recalls should be ascending.
...
class DetectionMetrics:
    ...
    def add_label_tp(self, detection_name: str, metric_name: str, tp: float) -> None:
    ...
    def tp_errors(self) -> Dict[str, float]:
        ...
        for metric_name in TP_METRICS:
            ...
            if np.isnan(class_errors).all():
                errors[metric_name] = np.nan
            else:
...
class DetectionBox(EvalBox):
    ...
    Arguments:
        ...
        rotation:
        velocity:
        ego_translation: Translation to ego vehicle in meters.
        num_pts: Nbr. LIDAR or RADAR inside the box. Only for gt boxes.
        detection_name: The class name used in the detection challenge.
        detection_score: GT samples do not have a score.
    ...
    def __init__(self,
        ...
        assert detection_name in DETECTION_NAMES, f'Error: Unknown detection_name {detection_name}'
    ...
    def deserialize(cls, content: Dict[str, Any]):
        ...
        if 'detection_score' not in content:
            ...
            detection_score = float(content['detection_score'])
...
class DetectionMetricDataList:
    ...
    def get_class_data(self, detection_name: str,
                       tag_name: str = None) -> List[Tuple[DetectionMetricData, float]]:
        """ Get all the MetricData entries for a certain detection_name. """
        if tag_name is not None:
            return [
                (md, dist_th) for (tag, name, dist_th), md in self.md.items()
                if name == detection_name and tag == tag_name
            ]

        return [
            (md, dist_th) for (_, name, dist_th), md in self.md.items()
            if name == detection_name
        ]

    def get_dist_data(self, dist_th: float,
                      tag_name: str = None) -> List[Tuple[DetectionMetricData, str]]:
        """ Get all the MetricData entries for a certain match_distance. """
        if tag_name is not None:
            return [
                (md, detection_name)
                for (tag, detection_name, dist), md in self.md.items()
                if dist == dist_th and tag == tag_name
            ]

        return [
            (md, detection_name)
            for (_, detection_name, dist), md in self.md.items()
            if dist == dist_th
        ]

    def set(self, tag_name: str, detection_name: str, match_distance: float,
            data: DetectionMetricData):
        """ Sets the MetricData entry for a certain detection_name and match_distance. """
        self.md[(tag_name, detection_name, match_distance)] = data
...
class DetectionMetricsList:
    """ This stores DetectionMetrics in a dict indexed by (tag-name). """
    def __init__(self):
        self.dm: Dict[str, DetectionMetrics] = {}
        self.eval_time = None
    ...
    def add_detection_metrics(self, tag_name: str, data: DetectionMetrics) -> None:
        self.dm[tag_name] = data

    def get_detection_metrics(self, tag_name: str) -> DetectionMetrics:
        return self.dm[tag_name]

    def add_label_ap(self, tag_name: str, *args, **kwargs) -> None:
        self.dm[tag_name].add_label_ap(*args, **kwargs)

    def get_label_ap(self, tag_name: str, *args, **kwargs) -> float:
        return self.dm[tag_name].get_label_ap(*args, **kwargs)

    def add_label_tp(self, tag_name: str, *args, **kwargs) -> None:
        self.dm[tag_name].add_label_tp(*args, **kwargs)

    def get_label_tp(self, tag_name: str, *args, **kwargs) -> float:
        return self.dm[tag_name].get_label_tp(*args, **kwargs)

    def add_runtime(self, eval_time: float) -> None:
        self.eval_time = eval_time

    def serialize(self) -> Dict[str, Any]:
        serialized = {k: v.serialize() for k, v in self.dm.items()}
        serialized['eval_time'] = self.eval_time
        return serialized
    ...
    def deserialize(cls, content: Dict[str, Any]):
        """ Initialize from serialized dictionary. """
        metrics_list = cls()
        metrics_list.eval_time = content.pop('eval_time', None)
        metrics_list.dm = {k: DetectionMetrics.deserialize(v) for k, v in content.items()}
        return metrics_list
